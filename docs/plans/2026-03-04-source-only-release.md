...
**Goal:** Publish `1.0.1` as the open-source Swift Package release and create the `develop` branch.

**Architecture:** Keep `Package.swift` as the only package distribution entrypoint, remove legacy packaging scaffolding, and update tests so they validate the source-based release model. Once the repository verifies cleanly, publish the new branch and tag, then create a GitHub release page with notes only.
...
Replace the old packaging contract assertions with source-only release assertions:
...
Expected: FAIL while the old packaging assertions are still in place
...
### Task 3: Remove legacy packaging scaffolding and keep source release metadata
...
Delete the wrapper package and legacy release scripts that are no longer part of the approved design.
...
Expected: GitHub release exists for tag `1.0.1` with notes only
